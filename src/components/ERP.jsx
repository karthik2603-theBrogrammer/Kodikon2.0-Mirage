...
function ERP() {
  ...
  const resetDB = () => {
    axios.get(url + "/cleardb").then(() => {
      console.log("Cleared The Database!");
    });
    axios
      .get(url)
      .then(() => {
        console.log("Refreshed the database!");
      })
      .catch((e) => {
        console.log(e);
      });
    ...
    axios
      ...
      .then((res) => {
        ...
        for (var i = 0; i < res?.data.length; i++) {
          if (res?.data[i]?.stock <= 5) {
            var data = {
              service_id: "service_c7rpmcp",
              template_id: " template_t42rru3",
              user_id: "ONcdngUzzsd0lkdBM",
              template_params: {
                username: "Karthik",
            ...
            axios
              ...
              .then((res) => console.log("Mail is sent"))
  ...
  return (
    <>
      ...
      <div className="bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 p-5 m-5 flex flex-row justify-center rounded-md">
        <div className="border-solid border-2 border-indigo-600 m-4 p-4">
          <h1 className="text-[20px] text-white self-center m-4">
          ...
          <img
            ...
            alt="NA"
            className="h-[200px] w-[200px] relative left-[15%] m-[22px] rounded-md bg-transparent"
          />
          ...
          <div className="flex items-center justify-center self-center flex-col text-white m-2 text-bold font-serif">
            {count}
            <p>Price: 1,20,000</p>
            <button
        ...
        <div className="border-solid border-2 border-indigo-600 m-4 p-4">
          <h1 className="text-[20px] text-white self-center m-4">
            Lenovo Yoga 9i 13th Gen (14, Intel)
          </h1>
          <img
            src="https://www.lenovo.com/medias/Yoga-Slim-7i-Pro-Gen-7-14-SS-Hero.png?context=bWFzdGVyfHJvb3R8NTkzNTJ8aW1hZ2UvcG5nfGhlMi9oYmMvMTY1Nzc3ODkwNjcyOTQucG5nfDRjOWI1YTY1YzBlOTI1ODI2NTNhNDEzYzY3ZTRlYTUwNDMxOTRiMGM3ZmI5NDQ3OThmNzQ0MDc3MGQzNjE0OTA"
            alt="NA"
            className="h-[200px] w-[200px] relative left-[40px] m-[22px] rounded-md"
          />
          <div className="w-[100%] flex items-center justify-center">
      ...
      <div>
        <div className="bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 p-5 m-5 flex flex-row justify-center rounded-md">
          <button
            onClick={handleChart}
            className="text-white mb-4 text-3xl font-extrabold text-gray-900 dark:text-white md:text-5xl lg:text-6xl"
          >
            GET CHART DATA
          </button>
        </div>
      </div>
      {gettingChart && <div>Loading...</div>}
      {showChart && (
        <div className="h-[60vh] flex flex-col justify-center items-center pt-[140px]">
          <Line data={data} options={options} className="m-[50px]" />
